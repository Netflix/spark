...
import org.apache.spark.sql.catalyst.plans.logical.{AlterTable, CreateTableAsSelect, CreateV2Table, DeleteFromTable, DescribeTable, DropTable, LogicalPlan, SubqueryAlias}
...
class PlanResolutionSuite extends AnalysisTest {
  ...
  object StringLiteral {
    def unapply(lit: Literal): Option[String] = {
      lit match {
  ...
  private val v2Format = "org.apache.spark.sql.connector.InMemoryTableProvider"
  ...
  private val testCat: TableCatalog = {
    ...
    when(newCatalog.loadTable(Matchers.any())).thenAnswer(new Answer[Table]() {
      override def answer(invocation: InvocationOnMock): Table = {
        invocation.getArgumentAt(0, classOf[Identifier]).name match {
          case "tab" =>
            table
          case "tab1" =>
            table1
  ...
  private val catalogManagerWithDefault = {
    ...
    when(manager.catalog(Matchers.any())).thenAnswer(new Answer[TableCatalog]() {
      override def answer(invocation: InvocationOnMock): TableCatalog = {
        invocation.getArgumentAt(0, classOf[String]) match {
          case "testcat" =>
            testCat
          case name =>
  ...
  test("DELETE FROM") {
    Seq("v2Table", "testcat.tab").foreach { tblName =>
      ...
      parsed2 match {
        case DeleteFromTable(_: DataSourceV2Relation,
